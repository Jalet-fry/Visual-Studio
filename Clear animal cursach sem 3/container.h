...
public:
    ...
    void change_this_creature(int index) {
        ...
        check_undo();
        // Сохраняем старое значение в истории перед изменением
        if (!is_undoing) {
            T old_value = (*it)->clone();
            history.push_back(Change(old_value, index, Operation::UPDATE));
        }
        (*it)->change_me();  // Обновляем существо
    }
    void undo() {
        if (history.empty()) {
        ...
        {
        case Operation::ADD: {
            // Если это добавление, нужно удалить элемент по индексу
            remove_at(last_change.index);
            break;
        }
        case Operation::REMOVE: {
            // Если это удаление, нужно добавить элемент обратно в список
    ...
    void save_to_file(string filename) const {
        ...
        try {
            if (is_empty()) {
                ...
                return;  // Если deque пуст, выходим из метода, не выполняя запись в файл
            }
            file.open(filename);
            if (!file.is_open()) {
            ...
            // Пропускаем ошибочные данные и продолжаем
        }
    }
    ...
    void search_creatures() {
        // Выводим доступные параметры поиска
        ...
        int choice = correct("Enter your choice: ", 1, 5);  // Вводим выбор поля поиска
        ...
        case 1: {  // Habitat
            ...
            // Перебираем все существа и ищем по полю habitat
            Iterator it = begin();
            while (it != end()) {
                Creature* creature = *it;
        ...
        case 2: {  // Age
            search_int = correct("Enter age: ", 0, 200);  // Вводим возраст

            // Перебираем все существа и ищем по полю age
            Iterator it = begin();
            while (it != end()) {
                Creature* creature = *it;
                if (Animal* animal = dynamic_cast<Animal*>(creature)) {
        ...
        case 3: {  // Web Type (для Spider и SpiderMan)
            cout << "Enter web type: ";
            getline(cin, search_string);

            // Перебираем все существа и ищем по полю web_type
            Iterator it = begin();
            while (it != end()) {
                Creature* creature = *it;
        ...
        case 4: {  // Leg Count (для Spider и SpiderMan)
            search_int = correct("Enter leg count: ", 0, 20);  // Вводим количество ног

            // Перебираем все существа и ищем по полю leg_count
            Iterator it = begin();
            while (it != end()) {
                Creature* creature = *it;
                if (Spider* spider = dynamic_cast<Spider*>(creature)) {
                    ...
                    // Для SpiderMan leg_count всегда равен 2
                    if (2 == search_int) {
                        cout << "Creature at index " << index << ":" << endl;
                        creature->show_me();
                    }
        ...
        case 5: {  // Profession (для Human, SpiderMan, Assassin)
            cout << "Enter profession: ";
            getline(cin, search_string);

            // Перебираем все существа и ищем по полю profession
            Iterator it = begin();
            while (it != end()) {
                Creature* creature = *it;
    ...
    /*
    Creature* breed_creatures(Creature* first, Creature* second) {
        if (Lion* lion = dynamic_cast<Lion*>(first) && Eagle * eagle = dynamic_cast<Eagle*>(second)) {
            return new Griffon(lion, eagle);
        }
        else if (Lion* lion = dynamic_cast<Lion*>(second) && Eagle * eagle = dynamic_cast<Eagle*>(first)) {
            return new Griffon(lion, eagle);

        }
        // Проверка для SpiderMan (Spider + Human)
        if (Human* human = dynamic_cast<Human*>(first) && Spider * spider = dynamic_cast<Spider*>(second)) {
            if (spider->get_leg_count() == 2) {
                return new SpiderMan(human, spider);
            }
            else {
        ...
        else if (Human* human = dynamic_cast<Human*>(second) && Spider* spider = dynamic_cast<Spider*>(first)) {
            if (spider->get_leg_count() == 2) {
                return new SpiderMan(human, spider);
            }
            else {
        ...
        // Проверка для Assassin (Human + Eagle)
        if (Human* human = dynamic_cast<Human*>(first) && Eagle* eagle = dynamic_cast<Eagle*>(second)) {
            int check_age = (human->get_age() + eagle->get_age()) / 2;
            // Сначала передаем Human и затем Eagle
            if (check_age >= 18 && check_age <= 50) {
                return new Assassin(human, eagle);
            }
            else {
        ...
        else if (Human* human = dynamic_cast<Human*>(second) && Eagle* eagle = dynamic_cast<Eagle*>(first)) {
            int check_age = (human->get_age() + eagle->get_age()) / 2;
            if (check_age >= 18 && check_age <= 50) {
                return new Assassin(human, eagle);
            }
        ...
        // Если не удалось найти подходящие типы существ для скрещивания
        else {
            throw Invalid_hybrid_exception("Как ты ни крути, но они не пара, не пара");
        }
    }
    */
    ...
    //Creature* breed_creatures(Creature* first, Creature* second) {
    //    // Проверка для Griffon (Lion + Eagle)
    //    if (Lion* lion = dynamic_cast<Lion*>(first)) {
    //        if (Eagle* eagle = dynamic_cast<Eagle*>(second)) {
    //            // Сначала передаем Lion и затем Eagle
    //            return new Griffon(lion, eagle);
    //        }
    //    }
    //    else if (Lion* lion = dynamic_cast<Lion*>(second)) {
    //        if (Eagle* eagle = dynamic_cast<Eagle*>(first)) {
    //            // Сначала передаем Lion и затем Eagle
    //            return new Griffon(lion, eagle);
    //        }
    //    }
    ...
    //    // Проверка для SpiderMan (Spider + Human)
    //    if (Human* human = dynamic_cast<Human*>(first)) {
    //        if (Spider* spider = dynamic_cast<Spider*>(second)) {
    //            if (spider->get_leg_count() == 2) {
    //                // Сначала передаем Human и затем Spider
    //                return new SpiderMan(human, spider);
    //            }
    //            else {
    //                throw Invalid_age_exception("У паука должно быть только 2 ноги");
    ...
    //        }
    //    }
    //    else if (Human* human = dynamic_cast<Human*>(second)) {
    //        if (Spider* spider = dynamic_cast<Spider*>(first)) {
    //            if (spider->get_leg_count() == 2) {
    //                // Сначала передаем Human и затем Spider
    //                return new SpiderMan(human, spider);
    //            }
