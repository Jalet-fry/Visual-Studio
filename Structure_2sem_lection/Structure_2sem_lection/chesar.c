//////////////////////////#include <stdio.h>
//////////////////////////#include <locale.h>
//////////////////////////#define ENG 26
//////////////////////////#define RUS 32
//////////////////////////void encrypt (int n)
//////////////////////////{
//////////////////////////    FILE *fp1, *fp2;
//////////////////////////    fp1 = fopen("input.txt", "r");
//////////////////////////    fp2 = fopen("output.txt", "w");
//////////////////////////    int flag;
//////////////////////////    char c;
//////////////////////////    c = getc(fp1);
//////////////////////////    while (!feof(fp1))
//////////////////////////    {
//////////////////////////        flag = 0; //обработан ли текущий символ
//////////////////////////        if (c >= 'A' && c <= 'Z')
//////////////////////////        {
//////////////////////////            c = c + (n % ENG);
//////////////////////////            if (c > 'Z') c = 'A' + (c - 'Z') - 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (c >= 'a' && c <= 'z')
//////////////////////////        {
//////////////////////////            c = c + (n % ENG);
//////////////////////////            if (c > 'z') c = 'a' + (c - 'z') - 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (c >= 'А' && c <= 'Я')
//////////////////////////        {
//////////////////////////            c = c + (n % RUS);
//////////////////////////            if (c > 'Я') c = 'А' + (c - 'Я') - 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (c>='а' && c<='я')
//////////////////////////        {
//////////////////////////            c = c + (n % RUS);
//////////////////////////            if (c > 'я') c = 'а' + (c - 'я') - 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (!flag) fprintf (fp2, "%c", c);
//////////////////////////        c = getc(fp1);
//////////////////////////    }
//////////////////////////    fclose (fp1);
//////////////////////////    fclose (fp2);
//////////////////////////}
//////////////////////////void decipher (int n)
//////////////////////////{
//////////////////////////    FILE *fp1, *fp2;
//////////////////////////    fp1 = fopen("input.txt", "r");
//////////////////////////    fp2 = fopen("output.txt", "w");
//////////////////////////    int flag;
//////////////////////////    char c;
//////////////////////////    c = getc(fp1);
//////////////////////////    while (!feof(fp1))
//////////////////////////    {
//////////////////////////        flag = 0;
//////////////////////////        if (c >= 'A' && c <= 'Z')
//////////////////////////        {
//////////////////////////            c = c - (n % ENG);
//////////////////////////            if (c < 'A') c = 'Z' - ('A' - c) + 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (c >= 'a' && c <= 'z')
//////////////////////////        {
//////////////////////////            c = c - (n % ENG);
//////////////////////////            if (c < 'a') c = 'z' - ('a' - c) + 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (c >= 'А' && c <= 'Я')
//////////////////////////        {
//////////////////////////            c = c - (n % RUS);
//////////////////////////            if (c < 'А') c = 'Я' - ('А' - c) + 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (c >= 'а' && c <= 'я')
//////////////////////////        {
//////////////////////////            c = c - (n % RUS);
//////////////////////////            if (c < 'а') c = 'я' - ('а' - c) + 1;
//////////////////////////            fprintf (fp2, "%c", c);
//////////////////////////            flag = 1;
//////////////////////////        }
//////////////////////////        if (!flag) fprintf (fp2, "%c", c);
//////////////////////////        c = getc(fp1);
//////////////////////////    }
//////////////////////////    fclose (fp1);
//////////////////////////    fclose (fp2);
//////////////////////////}
//////////////////////////int main ()
//////////////////////////{
//////////////////////////    //setlocale(LC_ALL,"Russian");
//////////////////////////    int n;
//////////////////////////    printf ("Введите натуральное n: ");
//////////////////////////    scanf ("%d", &n);
//////////////////////////    getchar (); //нужен для того, чтобы поймать символ клавиши ENTER, нажатой при вводе числа n
//////////////////////////    if (n < 1) return 0;
//////////////////////////    printf ("Чтобы зашифровать текст введите a, расшифровать b: ");
//////////////////////////    char c;
//////////////////////////    scanf ("%c", &c);
//////////////////////////    if (c == 'a') encrypt (n);
//////////////////////////    if (c == 'b') decipher (n);
//////////////////////////    return 0;
//////////////////////////}
///////////
///////////#include <stdio.h>
///////////#include <locale.h>
///////////#include <wchar.h>
///////////#define ENG 26
///////////#define RUS 32
///////////void encrypt (int n)
///////////{
///////////    FILE *fp1, *fp2;
///////////    fp1 = fopen("input.txt", "r");
///////////    fp2 = fopen("output.txt", "w");
///////////    int flag;
///////////    wchar_t c;
///////////    c = fgetwc(fp1);
///////////    while (!feof(fp1))
///////////    {
///////////        flag = 0; //обработан ли текущий символ
///////////        if (c >= L'A' && c <= L'Z')
///////////        {
///////////            c = c + (n % ENG);
///////////            if (c > L'Z') c = L'A' + (c - L'Z') - 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (c >= L'a' && c <= L'z')
///////////        {
///////////            c = c + (n % ENG);
///////////            if (c > L'z') c = L'a' + (c - L'z') - 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (c >= L'А' && c <= L'Я')
///////////        {
///////////            c = c + (n % RUS);
///////////            if (c > L'Я') c = L'А' + (c - L'Я') - 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (c>=L'а' && c<=L'я')
///////////        {
///////////            c = c + (n % RUS);
///////////            if (c > L'я') c = L'а' + (c - L'я') - 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (!flag) fputwc(c, fp2);
///////////        c = fgetwc(fp1);
///////////    }
///////////    fclose (fp1);
///////////    fclose (fp2);
///////////}
///////////void decipher (int n)
///////////{
///////////    FILE *fp1, *fp2;
///////////    fp1 = fopen("input.txt", "r");
///////////    fp2 = fopen("output.txt", "w");
///////////    int flag;
///////////    wchar_t c;
///////////    c = fgetwc(fp1);
///////////    while (!feof(fp1))
///////////    {
///////////        flag = 0;
///////////        if (c >= L'A' && c <= L'Z')
///////////        {
///////////            c = c - (n % ENG);
///////////            if (c < L'A') c = L'Z' - (L'A' - c) + 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (c >= L'a' && c <= L'z')
///////////        {
///////////            c = c - (n % ENG);
///////////            if (c < L'a') c = L'z' - (L'a' - c) + 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (c >= L'А' && c <= L'Я')
///////////        {
///////////            c = c - (n % RUS);
///////////            if (c < L'А') c = L'Я' - (L'А' - c) + 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (c >= L'а' && c <= L'я')
///////////        {
///////////            c = c - (n % RUS);
///////////            if (c < L'а') c = L'я' - (L'а' - c) + 1;
///////////            fputwc(c, fp2);
///////////            flag = 1;
///////////        }
///////////        if (!flag) fputwc(c, fp2);
///////////        c = fgetwc(fp1);
///////////    }
///////////    fclose (fp1);
///////////    fclose (fp2);
///////////}
///////////int main ()
///////////{
///////////    //setlocale(LC_ALL,"en_US.UTF-8");
///////////    int n;
///////////    wprintf (L"Введите натуральное n: ");
///////////    wscanf (L"%d", &n);
///////////    getchar (); //нужен для того, чтобы поймать символ клавиши ENTER, нажатой при вводе числа n
///////////    if (n < 1) return 0;
///////////    wprintf (L"Чтобы зашифровать текст введите a, расшифровать b: ");
///////////    wchar_t c;
///////////    wscanf (L"%lc", &c);
///////////    if (c == L'a') encrypt (n);
///////////    if (c == L'b') decipher (n);
///////////    return 0;
///////////}

